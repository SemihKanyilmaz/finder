...
func (r *postgresRepository) Upsert(ctx context.Context, contents []model.Content) error {
	...
	for i, c := range contents {
		...
		args = append(args, c.ID, c.Source, c.Title, string(c.Type), c.URL, c.PublishedAt, c.Views, c.Likes, c.ReadingTime, c.Reactions, c.Score)
	}

	query := fmt.Sprintf(`
		INSERT INTO contents (id, source, title, type, url, published_at, views, likes, reading_time, reactions, score)
		VALUES %s
...
func (r *postgresRepository) Search(ctx context.Context, params model.SearchParams) (model.SearchResult, error) {
	...
	if params.Keyword != "" {
		conditions = append(conditions, fmt.Sprintf(
			"search_vector @@ to_tsquery('simple', %s)", nextArg(params.Keyword),
		))
	}

	if params.ContentType != "" {
		conditions = append(conditions, fmt.Sprintf("type = %s", nextArg(string(params.ContentType))))
	}
	...
	query := fmt.Sprintf(
		`SELECT id, source, title, type, url, published_at, views, likes, reading_time, reactions, score, COUNT(*) OVER() AS total_count
		FROM contents %s %s LIMIT %s OFFSET %s`,
		where, orderBy, nextArg(params.PageSize), nextArg(offset),
	)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.Source, &c.Title, &contentType, &c.URL, &c.PublishedAt, &c.Views, &c.Likes, &c.ReadingTime, &c.Reactions, &c.Score, &totalCount); err != nil {
